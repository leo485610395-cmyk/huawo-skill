...
description: 帮用户做个人介绍网站的 Claude Skill。通过对话收集用户信息,自动填进 HTML 模板,交付完整可用的网站。内置七种风格(水墨安静风/极简现代风/温暖手作风/暗色高级风/瑞士国际风/杂志编辑风/新粗野风),纯 HTML/CSS 无依赖,中文优先,克制不炫技。当用户说「帮我做个人网站」「我有个模板要填」「做一个 portfolio」「个人介绍站」或把模板文件拖给 AI 时触发。
author: 化我
version: 1.0.0
license: MIT
...
帮用户把模板变成网站。用户已经下载了 7 个 HTML 模板之一(或者还没选)。你通过对话帮用户:
...
简短介绍 7 个风格(每条一句话,**别堆大段描述**,用户没耐心读;一次列全 7 个太长的话,可以先问用户做什么的,再挑 3-4 个最匹配的推荐):
...
> - **暗色高级风**(`assets/05-暗色高级风.html`)—— 近黑深底、衬线大字、居中画廊式排版、一点金色、噪点质感、画廊式进场淡入。适合独立开发者、设计师、摄影师
> - **瑞士国际风**(`assets/06-瑞士国际风.html`)—— 无衬线细字重大标题、克莱因蓝、非对称网格拼贴、十字准星、模块卡、精确上升进场。适合工程师、产品经理、技术人设
> - **杂志编辑风**(`assets/07-杂志编辑风.html`)—— 报头、双细线、首字下沉、栏线+尾花、报纸版面、油墨式淡入。适合写作者、内容创作者
> - **新粗野风**(`assets/08-新粗野风.html`)—— 奶油底、粗黑边框、硬阴影、黄红撞色、跑马灯。适合年轻创作者、独立艺术家
>
...
2. **「怎么用」不靠用户看文档,靠你一句句带出来**。交付不是甩个文件加一屏说明,是一次带一步,用户做完一步、回了话,再带下一步

#### 带的节奏(一次一步,等反馈)

1. 第一步只说一句:「文件在 [路径],双击打开,先看首屏——进场有动效,看完说下感觉」
2. 用户回了,再带:往下滚一屏看板块进场;顺手用手机或浏览器 F12 手机视图看一眼,确认不乱
3. 换图**带着换第一张**:让用户把一张图放到 HTML 同目录、告诉你文件名,你改好 `src` 让他刷新确认——第一张带着换完,剩下的他自己就会了
4. 动效不用用户配置,但主动交代:进场/滚动动画是全自动的;他嫌多嫌少,你直接替他调(每个模板头部注释写了动效边界,照着调,别越界)
5. 想上线**再**带上线:Netlify(免费),注册 → 拖文件夹,一步一步等他做完再给下一步;没提上线就不主动推
6. 还有没填的占位符,在这个阶段告诉他哪些保留着、怎么补(内容发给你,你来改)
...
双击用浏览器打开,先看第一屏——上面几行字是依次进场的,往下滚每个板块也会自己浮现。
...
- 用户卡在哪,直接替他做(改文件、改链接),不要只给说明让他自己操作
- 模板头部注释里的【使用方法】【不要改的部分】是给你看的,用户不用读;用户问什么,你答什么
- 别一口气把「换图+上线+手机端」全倒出来——一次一步,他在跟着走的节奏里自然学会这个站怎么用
...
> 模板包里有七个风格:水墨/极简/温暖手作/暗色高级/瑞士国际/杂志编辑/新粗野。
> 你要的 [XXX 风] 我没有现成模板。
...
- ❌ **不要一次性倒使用说明**(交付后带着用户一步步用,一次一步,等他反馈再走下一步)
- ❌ **不要混风格**(比如把水墨元素加到极简站里)
...
模板有固定的作品槽位(温暖手作风 6 个,极简现代风 4 个,水墨安静风 3 个,暗色高级风 4 个,瑞士国际风 4 个,杂志编辑风 4 个,新粗野风 4 个)。用户的作品数量**可能不够**。
...
2. **用户作品 < 模板槽位**:有两种处理方式,优先用方式 A
   - **方式 A(推荐)**:删掉多余的作品卡。删整块 `<div class="work-card">...</div>` 或对应 class 的整块。
     ...
     - **注意检查 CSS 有没有用 `nth-child` 控制错位/配色轮换**(温暖手作风用了)。**删卡安全的例外**:瑞士国际风的作品是 2×2 作品格(没有配色轮换,删任意一格都不乱,记得把保留格的编号重排成 01 连续)、暗色高级风的作品是全宽单幅纵向堆叠(删任意一幅都不乱)、杂志编辑风删末尾简讯式小作品即可(头条作品别删,它是版面锚点)、新粗野风的作品卡配色轮换是 `nth-child(odd/even)`(奇偶交替,删中间任意一张都不乱),这几个删卡放心删。其他风格删卡可能错位,让用户补作品或换风格
...
`assets/` 里的文件编号是 `01 / 02 / 04 / 05 / 06 / 07 / 08`,**没有 03**。这是因为 03 是早期废弃的版本(暗黑终端风,不属于本 skill 范围),故意跳过的,不是漏文件。

用户问起就说:「03 是早期版本,跳过了,其余 7 个就是完整的风格清单」。
...
- [`assets/05-暗色高级风.html`](assets/05-暗色高级风.html) —— 暗色高级风模板
- [`assets/06-瑞士国际风.html`](assets/06-瑞士国际风.html) —— 瑞士国际风模板
- [`assets/07-杂志编辑风.html`](assets/07-杂志编辑风.html) —— 杂志编辑风模板
- [`assets/08-新粗野风.html`](assets/08-新粗野风.html) —— 新粗野风模板
